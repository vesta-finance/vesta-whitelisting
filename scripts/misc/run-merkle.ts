...
async function main() {
	let whitelist: { account: string; amount: BigNumber }[] = [
		//TEST ACCOUNTS
		{
	...
	let b: BigNumber = BigNumber.from("0")
	whitelist.forEach(element => {
		b = b.add(element.amount)
	})
...
function outputProofKey(
	...
	for (let i = start; i < end; i++) {
		console.log(bt.getProof(whitelist[i].account, whitelist[i].amount))
		console.log("")
	}
}
